...
The core of the spatial extension is the [`GEOMETRY` type]({% link docs/preview/sql/data_types/geometry.md %}), which is a flexible and extensible data type for representing geometric objects. The `GEOMETRY` type used to be provided by the `spatial` extension, but it became a built-in data type in DuckDB v1.5. Almost all of the associated functions for working with geometries (e.g., calculating distances, areas, intersections) are still part of `spatial` though.
